...
# The targets here do not use dependencies, so it's a strange use of
# `make'.  Really, this Makefile is an alternative to a pile of
...
core:
	cd racket/src; mkdir -p build
	cd racket/src/build; ../configure
	cd racket/src/build; make
	cd racket/src/build; make install
...
native-from-git:
	mkdir -p build
	if [ ! -d build/native-pkgs ]; then cd build; git clone git://github.com/mflatt/libs.git native-pkgs ; fi

...
(define-values (N total-futures) (command-line #:args (n f) (values (string->number n) (string->number f))))
(define N.0 (fx->fl N))
...
(define stride (/ N total-futures))

;(visualize-futures
(for-each
 touch
 (for/list ([y* (in-range N 0 (- stride))])
   (future
    (λ ()
      (for ([y (in-range y* (- y* stride) -1)])

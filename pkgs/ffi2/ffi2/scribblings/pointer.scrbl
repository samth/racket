...
@deftech{pointer subtype}. A @racket[void_t*] representation has no tags,
which means that a C conversion to Racket via @racket[void_t*] is not
accepted by any context that expects some tag, whereas a pointer with
any tags is accepted as a @racket[void_t*] representation to translate
to C. Whether a pointer object represents an address managed by
Racket's garbage collector is independent of its tags.

@deftogether[(
@defproc[(ffi2-ptr? [v any/c]) boolean?]
@defproc[(ffi2-ptr/gcable? [v any/c]) boolean?]
)]{

The @racket[ffi2-ptr?] predicate recognizes any @tech{pointer} object,
while @racket[ffi2-ptr/gcable?] recognizes a pointer object that
represents a reference to memory that is managed by Racket's garbage
collector.
...
@defproc[(ffi2-free [ptr ffi2-ptr?]) void?]{

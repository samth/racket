...
(for ([i (in-range 10)])
  (define-ffi2-type percentage_box_t/gcable void_t*/gcable
  ...
  (let ()
    ...
    (check-equal? (ffi2-ref (ffi2-ref p void_t*/gcable) double_t) 0.505)))
...
(for ([i (in-range 10)])
  ...
  (let ()
    ...
    (check-equal? (ffi2-ref (ffi2-ref p void_t*) double_t) 0.505)
    (void (black-box p))))

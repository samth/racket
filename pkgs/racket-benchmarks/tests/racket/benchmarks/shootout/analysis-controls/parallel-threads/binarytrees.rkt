...
(require racket/cmdline racket/unsafe/ops racket/future)
...
(define (main n)
  ...
  (define workers (min 4 (processor-count) (length tasks)))
  (define groups
    (for/list ([worker (in-range workers)])
      (for/list ([task (in-list tasks)] [i (in-naturals)]
                 #:when (= (modulo i workers) worker))
        task)))
  (define pool (and (> workers 1) (make-parallel-thread-pool (sub1 workers))))
  (define threads
    (for/list ([tasks (in-list (cdr groups))])
      (thread #:pool pool #:keep 'results (lambda () (run-tasks tasks)))))
    ...
    (append own-results
            (apply append
                   (for/list ([worker (in-list threads)])
                     (thread-wait worker
                       (lambda () (error 'binarytrees "parallel worker failed")))))))

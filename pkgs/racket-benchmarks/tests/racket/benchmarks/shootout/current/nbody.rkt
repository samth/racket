...
(define (parse-count text)
  ...
  (unless (and (exact-nonnegative-integer? n) (fixnum? n))
    (raise-user-error 'nbody "expected a nonnegative fixnum iteration count; received ~a" text))
  n)
...
(let ([n (command-line #:args (n) (parse-count n))])
  (offset-momentum)

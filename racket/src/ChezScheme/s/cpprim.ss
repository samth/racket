    ...
    (let ()
      ...
      (let ()
        ...
        (let ()
          ...
          ; For a positive divisor d = 2^k, (modulo x d) is (logand x (- d 1))
          ; in two's complement, negative x included, and that holds of the
          ; tagged representation directly: (x mod d) * fixnum-factor is
          ; (x * fixnum-factor) masked with ((d - 1) * fixnum-factor).  It is
          ; the rule fxmod above already uses.  Any other divisor falls back to
          ; the library procedure, whose cost next to the division itself is
          ; small.
